...
class Dashboard {
public:
    Dashboard() : start_time_(get_current_timestamp()), last_update_(get_current_timestamp()) {}
    
    void run() {
        std::string line;
        last_update_ = get_current_timestamp();
        ...
        while (std::getline(std::cin, line)) {
            ...
            if (now - last_update_ >= 2000) { // 2 seconds
                display_dashboard();
                last_update_ = now;
            }
        }
...
private:
    ...
    void update_metrics() {
        long long now = get_current_timestamp();
        long long elapsed_ms = now - start_time_;
        
        // Calculate true camera capture rate (frames per second)
        if (!camera_frame_timings_.empty() && elapsed_ms > 0) {
        ...
        if (elapsed_ms > 0) {
            current_metrics_.ips = static_cast<float>(inference_count_) / (elapsed_ms / 1000.0f);
        } else {
            current_metrics_.ips = 0.0f;
        }
        
        // For now, we'll set other metrics to N/A values
