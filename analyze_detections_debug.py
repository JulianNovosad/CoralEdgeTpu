...
for line in lines:
    ...
    # Format: [INFO] DETECTION_DISTANCE: class=90, score=12.000000, box=[5.380857,4.035642,321.506165,309.399231], distance=0.371946m
    match = re.search(r'DETECTION_DISTANCE: class=(\d+), score=([0-9.]+), box=$$([0-9.,-]+)$$, distance=([0-9.]+)m', line)
    if match:
...
print(f"Parsed {len(parsed_data)} detections from {len(lines)} lines")

if len(parsed_data) == 0:
    print("Debug: Showing first few lines for pattern matching:")
