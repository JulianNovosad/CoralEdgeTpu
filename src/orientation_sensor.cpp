...
OrientationSensor::OrientationSensor(unsigned short yaw_port, unsigned short pitch_port, unsigned short roll_port)
    ...
    APP_LOG_INFO("OrientationSensor created (mock data provider). Yaw Port: " + std::to_string(yaw_port) +
...
void OrientationSensor::worker_thread_func() {
    ...
    APP_LOG_INFO("OrientationSensor worker thread started (mocking data).");
    while (running_) {
